...
parser_cmd_session_add.add_argument("-u", "--user", metavar = "user")
...
parser_scrobble.add_argument("artist", metavar = "artist")
parser_scrobble.add_argument("track", metavar = "track")
parser_scrobble.add_argument("timestamp", metavar = "timestamp",
                              help = "\n{0}Time of scrobbling. Can be formatted with -f,\n{0}otherwise it's " \
                              "an UNIX timestamp.".format("\b" * 18))
parser_scrobble.add_argument("-f", "--format", metavar = "format", action = ParserScrobbleFormatAction,
                              help = "{0}Specifies the format of the timestamp. Uses\n{0}the same syntax " \
                              "as Python's strftime().".format("\b" * 18))
parser_scrobble.add_argument("-a", "--album", metavar = "album")
parser_scrobble.add_argument("-d", "--duration", metavar = "duration")
parser_scrobble.add_argument("-m", "--mbid", metavar = "mbid")
parser_scrobble.add_argument("-t", "--track-number", metavar = "track_number", dest = "tracknumber")
parser_scrobble.add_argument("-aa", "--album-artist", metavar = "album_artist", dest = "albumartist")
parser_scrobble.add_argument("-s", "--stream-id", metavar = "stream_id", dest = "streamid")
parser_scrobble.add_argument("-c", "--chosen-by-user", action = "store_true", dest = "chosenbyuser")
parser_scrobble.add_argument("-cx", "--context", metavar = "context")
...
parser_cmd_unp.add_argument("artist", metavar = "artist")
parser_cmd_unp.add_argument("track", metavar = "track")
parser_cmd_unp.add_argument("-a", "--album", metavar = "album")
parser_cmd_unp.add_argument("-d", "--duration", metavar = "duration")
parser_cmd_unp.add_argument("-m", "--mbid", metavar = "mbid")
parser_cmd_unp.add_argument("-t", "--track-number", metavar = "track_number", dest = "tracknumber")
parser_cmd_unp.add_argument("-aa", "--album-artist", metavar = "album_artist", dest = "albumartist")
parser_cmd_unp.add_argument("-cx", "--context", metavar = "context")
